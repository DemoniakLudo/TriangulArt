...
namespace TriangulArt {
	public partial class EditColor : Form {
			...
			if (cpcPlus) {
				...
				for (int y = 0; y < 3; y++)
					for (int x = 0; x < 9; x++) {
						...
						colors[i].Tag = i;
						colors[i].BackColor = Color.FromArgb(PaletteCpc.RgbCPC[i].GetColorArgb);
		...
		private void NewColor() {
			int v = tabVal[0] != null ? Utils.ToInt(tabVal[0].Text) : 0;
			int r = tabVal[1] != null ? Utils.ToInt(tabVal[1].Text) : 0;
