	...
	public class GenereAsm {
		...
		static private void GenereInitCode(StreamWriter sw, int mode, bool cpcPlus) {
			...
			if (mode == 1) {
				...
				sw.WriteLine("	LD	A,16");
				sw.WriteLine("InitPen:");
				sw.WriteLine("	LD	BC,5");
				sw.WriteLine("	LDIR");
				sw.WriteLine("	INC	E");
				sw.WriteLine("	INC	E");
				sw.WriteLine("	INC	E					; Alignement sur 8 octets");
				sw.WriteLine("	DEC	A");
				sw.WriteLine("	JR	NZ,InitPen");
			}
			if (cpcPlus) {
		...
		static private void GenereDrawTriangle(StreamWriter sw, int mode) {
			...
			if (mode == 1) {
				...
				sw.WriteLine("	RLA	");
				sw.WriteLine("	RLA	");
				sw.WriteLine("	RLA							; * 8 (8 octets par couleurs)");
				sw.WriteLine("	LD	(DrawLigneCoul+1),A");
				sw.WriteLine("	ADD	A,4");
				sw.WriteLine("	LD	(DrawLigneCoul4+1),A");
			}
			...
			sw.WriteLine("	LD	L," + (mode == 1 ? "255" : "128"));
			sw.WriteLine("	LD	A,L");
			...
			if (mode == 1) {
				...
				sw.WriteLine("	JR	NZ,DrawLigneCoul2				; pixel droite => normal");
				sw.WriteLine("	LD	A,B");
				sw.WriteLine("	INC	A");
				sw.WriteLine("	SUB	C");
				sw.WriteLine("	JR	Z,DrawLigneCoul2");
				sw.WriteLine("	DEC	A");
				sw.WriteLine("	JR	DrawLigneCoul3");
				sw.WriteLine("DrawLigneCoul2");
				sw.WriteLine("	LD	A,(DE)					; Octet memoire ecran");
				sw.WriteLine("	AND	(HL)					; Masque");
				sw.WriteLine("	INC	L");
				...
				sw.WriteLine("	LD	A,B");
				sw.WriteLine("	RRCA");
				sw.WriteLine("	JR	C,DrawLigneCoul7");
				sw.WriteLine("	INC	E");
				sw.WriteLine("	RLCA");
				sw.WriteLine("	SUB	C");
				sw.WriteLine("DrawLigneCoul3");
				sw.WriteLine("	NEG");
				sw.WriteLine("	CP	2");
				sw.WriteLine("	JR	C,DrawLigneCoul6");
				sw.WriteLine("	RRA");
				...
				sw.WriteLine("	LD	L,0");
				sw.WriteLine("	LDI");
				sw.WriteLine("	JP	PO,DrawLigneCoul5			; Si Seulement 1 octet a copier (bc=0)");
				sw.WriteLine("	LD	H,D");
				sw.WriteLine("	LD	L,E");
				sw.WriteLine("	DEC	L					; Possible car taille ecran=64 caracteres");
				sw.WriteLine("	LDIR");
				...
				sw.WriteLine("	LD	B,A");
				sw.WriteLine("	JR	DrawLigneCoul8");
				sw.WriteLine("DrawLigneCoul6");
				sw.WriteLine("	LD	A,B");
				sw.WriteLine("	JR	DrawLigneCoul8");
				sw.WriteLine("DrawLigneCoul7");
				sw.WriteLine("	RLCA						;x1 * 2");
				sw.WriteLine("DrawLigneCoul8");
			...
			sw.WriteLine("	" + (mode == 1 ? "JR" : "JP") + "	C,BclDrawTriangle");
		...
		static public void GenereDrawTriangleData(StreamWriter sw, int mode, bool cpcPlus) {
			...
			if (mode == 1) {
				...
				sw.WriteLine("	DB	#55,#00,#AA,#00,#00");
				sw.WriteLine("	DB	#55,#80,#AA,#40,#C0");
				sw.WriteLine("	DB	#55,#08,#AA,#04,#0C");
				sw.WriteLine("	DB	#55,#88,#AA,#44,#CC");
				sw.WriteLine("	DB	#55,#20,#AA,#10,#30");
				sw.WriteLine("	DB	#55,#A0,#AA,#50,#F0");
				sw.WriteLine("	DB	#55,#28,#AA,#14,#3C");
				sw.WriteLine("	DB	#55,#A8,#AA,#54,#FC");
				sw.WriteLine("	DB	#55,#02,#AA,#01,#03");
				sw.WriteLine("	DB	#55,#82,#AA,#41,#C3");
				sw.WriteLine("	DB	#55,#0A,#AA,#05,#0F");
				sw.WriteLine("	DB	#55,#8A,#AA,#45,#CF");
				sw.WriteLine("	DB	#55,#22,#AA,#11,#33");
				sw.WriteLine("	DB	#55,#A2,#AA,#51,#F3");
				sw.WriteLine("	DB	#55,#2A,#AA,#15,#3F");
				sw.WriteLine("	DB	#55,#AA,#AA,#55,#FF");
			}
			sw.WriteLine(";");
